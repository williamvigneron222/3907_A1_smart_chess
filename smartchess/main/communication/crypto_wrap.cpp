...
#define MAX_WORDS 64  // 64 x 8 bytes = 512 bytes max
                         ...
                         size_t *ct_bytes_out)
{
    ...
    // Pad up to next 16-byte boundary (ASCON block size)
    ...
    // Copy samples into zero-padded word buffer
    ...
    ascon_encrypt(KEY, nonce_out, nullptr, 0,
                         ...
                         size_t *samples_out_count, size_t original_samples)
{
    ...
    ascon_decrypt(KEY, const_cast<uint64_t*>(nonce),
                  nullptr, 0,
