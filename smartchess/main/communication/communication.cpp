...
// !! Fill in the MAC address of the peer device !!
// Run communication_setup() on the other ESP32 and read
// the serial log: "My STA MAC: XX:XX:XX:XX:XX:XX"
                           ...
                           const uint8_t *data, int len)
{
    ...
    // Hand decrypted audio to firmware for playback
    if (g_rx_cb) {
...
void communication_setup(audio_rx_cb_t rx_callback)
{
    ...
    wifi_init_sta();
    espnow_init_common();
    local_crypto_test();   // verify crypto works on boot
...
void communication_send(const int16_t *samples, size_t num_samples)
{
    ...
    if (num_samples > PAYLOAD_MAX_SAMPLES) {
        ESP_LOGW(TAG, "Audio block too large: %u (max %d)",
