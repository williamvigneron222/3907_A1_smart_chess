...
%     \draw [->] ($(0,1.5cm)+(extra)$)node[above]{$d_{\beta 2}$} -- (extra);
    ...
    \node [block, fill=Goldenrod] (digital){digitize()};
    \node [block, right of=digital, fill=Goldenrod] (packet){packetize()};
    \node [block, right of=packet, fill=ProcessBlue, node distance=3cm] (crypto){encrypt()};
    \node [block, right of=crypto, fill=ProcessBlue, node distance=3cm] (send){send()};

    \node [block, above of=crypto, fill=ProcessBlue, node distance=2cm] (getkey){getKey()};

    \node [block, above of=getkey, fill=OrangeRed, node distance=2cm] (rng){getRandomBits()};

    \draw [->] (digital) -- (packet);
    \draw [->] (rng) -- (getkey);
    \draw [->] (crypto) -- node{$n\cdot 64b'$} (send);
    \draw [->] (packet) -- node{$n\cdot 64b'$} (crypto);
    \draw [->] (getkey) -- node{$128b'$} (crypto);


    \node [block, below of=digital, fill=ProcessBlue, node distance=5cm] (recv){recieve()};
    \node [block, right of=recv, fill=ProcessBlue] (decrypto){decrypt()};
